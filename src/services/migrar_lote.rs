...
pub async fn migrar_lote() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let count: i64 = conn
        .exec_first(
            "SELECT COUNT(*) from tbl_docDigitalizados",
            (),
        )?
        .unwrap();
    ...
    let mut start = 0;
    // let batch_size = 5;
    let batch_size: i64 = lote.parse().unwrap();
    while start < count {
        let query = format!("SELECT * from tbl_docDigitalizados WHERE eMigrado_fl = 'N' ORDER BY lDocDigitalizado_id ASC LIMIT {}, {}", start, batch_size);
