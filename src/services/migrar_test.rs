...
pub async fn migrar_test() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let limite: i64 = limite_test.parse().unwrap();
    ...
    let query = format!("SELECT * from tbl_docDigitalizados WHERE eMigrado_fl = 'N' ORDER BY lDocDigitalizado_id ASC LIMIT {}", limite);
    ...
    let db: Database = client.database("pagoalpaso_demo");
    let collection = db.collection("tbl_docDigitalizados_test");
    let collection_byte = db.collection("tbl_docDigitalizados_byte_test");
